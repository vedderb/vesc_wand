...
)
{
    ...
    // Write it to the SPI
    deca_port_writetospi(cnt,header,length,buffer);
...
)
{
    ...
    deca_port_readfromspi(cnt, header, length, buffer);  // result is stored in the buffer
...
void _dwt_loaducodefromrom(void)
{
    ...
    deca_port_sleep(1); // Allow time for code to upload (should take up to 120 us)
...
uint16 dwt_readtempvbat(uint8 fastSPI)
{
    ...
    if(fastSPI == 1)
    {
        deca_port_sleep(1); // If using PLL clocks(and fast SPI rate) then this sleep is needed
